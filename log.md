...
- **Retired RESOLVER.md rules 15, 16, 19** (partnership, agency, inbox) — confirmed with Dave that the partnership/agency process is genuinely discontinued, not just the documents being stale. Rule numbers kept in place (not renumbered/deleted) so existing historical references in this log (rule 5, 6, 11, 14, 17) stay accurate. Rule 19's inbox/ catch-all is retired with no replacement folder — misfit documents now get flagged directly to Dave for manual triage instead. Rule 8a's cross-reference to rule 15 updated to reflect the new discontinued status.
