...
### Granularity for Skill Candidates
Skill-candidate documents follow a different granularity principle than the File Granularity rules above, because of how they get consumed:

**One skill = one trigger = one unit loaded whole.** Unlike passive retrieval (where a `##` chunk being returned in isolation is fine — that's the point of chunking), executing a skill needs its full sequence of steps + guardrails together. If step 5 gets retrieved without steps 1-4 and the guardrails, an agent following it literally will execute incorrectly. So:

1. **Don't split a single triggerable procedure across separate files.** If the JOB STEPS + GUARDRAILS for one flow need to be seen together to execute correctly, they stay together as one complete unit — never split "because it's long."
2. **A file bundling multiple flows with genuinely different trigger conditions is a future-splitting candidate, not a now-splitting one.** Example: `sales-halo-ai-agent-spec.md` currently bundles 3 distinct flows (Triage, Akademi Sales, Sekolah Musik) each with its own trigger condition — a strong candidate to eventually become 3 separate skill files so an agent executing the Sekolah Musik flow doesn't have to load the unrelated Akademi steps too. Don't split it now — that's a decision for whenever the skill-execution system is actually being built (it may need a different format entirely), not for the current RAG-only phase.
3. **The ~500-word chunk cap (Chunking Convention above) is a retrieval-mode rule, not a skill-loading rule.** The same file gets consumed two different ways: RAG retrieval chunks at `##` boundaries with a 500-word cap (for answering questions with relevant context), while skill-loading (once built) pulls the entire relevant flow regardless of that cap (for executing a complete procedure). Don't shrink a flow's steps to fit the 500-word retrieval limit — that limit doesn't apply to the skill-loading consumption path.
